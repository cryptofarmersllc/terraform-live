...
scp id_rsa id_rsa.pub config validator2.cryptofarmers.io:/home/groot/.ssh/
#verify ssh connection is established from destination to source node
ssh use2lvalidator001prod -p 1122
#Rsync beacon node database from source to destination node while beacon node is running
rsync --progress -avzhe "ssh -p 1122" use2lvalidator001prod:/data/ethereum/beaconchaindata/beaconchain.db .
#Rsync beacon node database from source to destination node while beacon node is stopped
rsync --progress -avzhe "ssh -p 1122" use2lvalidator001prod:/data/ethereum/beaconchaindata/beaconchain.db .